...
public:
     ...
     * @param robotFile The full path the the robot SDF file
    ...
    /**
     * @brief Initializes the propagator plugin
     *
     * @param pluginFile The file to load the plugin from
     * @param robotEnvironment A pointer to the RobotEnvironment that is passed to the 'load'
     * function of the plugin
     */
    void initTransitionPlugin(const std::string& pluginFile,
                              RobotEnvironment* robotEnvironment);

    /**
     * @brief Loads the initialized propagator plugin
     */
    void loadTransitionPlugin(const std::string& optionsFile,
                              RobotEnvironment* robotEnvironment);

    /**
     * @brief Initializes the observation plugin
     *
     *
     * @param pluginFile The file to load the plugin from
     * @param processError The observation error that will be used inside the propagator plugin
     * @param robotEnvironment A pointer to the RobotEnvironment that is passed to the 'load'
     * function of the plugin
     *
     */
    void initObservationPlugin(const std::string& pluginFile,
                               RobotEnvironment* robotEnvironment);

    /**
     * @brief Loads the initialized observation plugin
     */
    ...
    virtual void updateViewer(const oppt::RobotStateSharedPtr& state,
                              ...
                              const VectorFloat& particleColor = VectorFloat(),
     ...
     */
    const std::string getRobotFile() const;
    ...
    std::pair<bool, VectorFloat> getIKSolution(const VectorFloat &pose,
...
protected:
    ...
    std::unique_ptr<TransitionPlugin> propagator_;
    ...
    std::unordered_map<std::string, std::vector<GeometrySharedPtr>> linkCollisionGeometryMap_;

    std::vector<OpptCollisionObjectSharedPtr> robotCollisionObjectsVec_;

    std::vector<OpptCollisionObjectSharedPtr> invariantRobotCollisionObjectsVec_;
...
protected:
    ...
    virtual void addObstacleCallback(oppt::ObstacleSharedPtr& obstacle);

    virtual void removeObstacleCallback(std::string name);

    virtual void changeObstaclePoseCallback(const std::string& name, const geometric::Pose& poseVec);
    ...
    virtual void registerOnAddObstacleCallback(std::function<void(const std::string&)>& callback);

    virtual void registerOnRemoveObstacleCallback(std::function<void(const std::string&)>& callback);
...
private:
    ...
    bool initializeViewer_(std::string modelFile, std::string environmentFile);

    virtual void setupViewer(std::string modelFile, std::string environmentFile);
