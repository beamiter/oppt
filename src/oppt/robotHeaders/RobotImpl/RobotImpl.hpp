...
{
public:
    RobotImpl(std::string worldFile,
...
protected:
    virtual void addObstacleCallback(oppt::ObstacleSharedPtr& obstacle) override;

    virtual void removeObstacleCallback(std::string name) override;

    virtual void changeObstaclePoseCallback(const std::string& name, const geometric::Pose& poseVec) override;
...
protected:
    ...
    virtual void registerOnAddObstacleCallback(std::function<void(const std::string&)>& callback) override;

    virtual void registerOnRemoveObstacleCallback(std::function<void(const std::string&)>& callback) override;
