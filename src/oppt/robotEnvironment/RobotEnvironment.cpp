...
void RobotEnvironment::initializeEnvironmentCallbacks()
{
    if (prefix_ == "exec") {
        ...
        onObstaclePoseChangedFn_ =
            std::function<void(const std::string&, const geometric::Pose&)>(std::bind(&RobotEnvironment::onObstaclePoseChanged,
                    this,
                    std::placeholders::_1,
                    std::placeholders::_2));
...
void RobotEnvironment::onObstaclePoseChanged(const std::string& str, const geometric::Pose& pose)
{
    // This method gets registered and called in GazeboInterface to notify the environment
    // About changes made in the gazebo client of the exec environment
    if (prefix_ == "exec") {
...
void RobotEnvironment::applyChanges()
{
    ...
    while (change) {
        ...
        case EnvironmentChangeType::OBSTACLE_POSE_CHANGED: {
            ...
            const geometric::Pose pose = change->as<ObstaclePoseChange>()->getPose();
...
void RobotEnvironment::onEnvironmentChanged(const EnvironmentChangeSharedPtr& environmentChange)
{
    ...
    case EnvironmentChangeType::OBSTACLE_POSE_CHANGED: {
        ...
        changeObstaclePose(obstacleName, 
            environmentChange->as<ObstaclePoseChange>()->getPose(), 
...
bool RobotEnvironment::changeObstaclePose(const std::string& name, const geometric::Pose& pose, bool executeCallbacks)
{
    bool poseChanged = environmentInfo_->getScene()->changeObstaclePose(name, pose, executeCallbacks);
#ifdef USE_RVIZ
    auto viewer = robot_->getViewer();
    if (viewer)
        static_cast<ViewerPublisher *>(viewer)->updateFromEnvironmentInfo(environmentInfo_);
