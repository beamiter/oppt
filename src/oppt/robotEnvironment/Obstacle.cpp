...
ObstacleImpl::ObstacleImpl(const std::string& name, const geometric::Pose &worldPose):
    oppt::Obstacle(name, worldPose)
{

}

void ObstacleImpl::createCollisionObject() {
    ...
    auto collisionObject = CollisionObjectSharedPtr(new fcl::CollisionObject(collisionGeometry, transform));
    opptCollisionObject_ = OpptCollisionObjectSharedPtr(new OpptCollisionObject(collisionObject, getName()));
}


void ObstacleImpl::updateCollisionObject() {
...
void ObstacleImpl::initVisualGeometry(const GeometrySharedPtr &visualGeometry) {
...
bool ObstacleImpl::collides(const std::vector<oppt::ObstacleSharedPtr>& otherObstacles) const
{
    ...
    for (size_t i = 0; i != otherObstacles.size(); ++i) {
        collisionObjects[i] = otherObstacles[i]->getOpptCollisionObject()->getCollisionObject();
    }
