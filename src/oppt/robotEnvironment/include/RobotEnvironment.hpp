...
public:
    ...
    void loadRewardPlugin(const std::string& pluginFile,
                          const std::string& optionsFile);

    void loadTerminalPlugin(const std::string& pluginFile,
                            const std::string& optionsFile);

    void loadInitialBeliefPlugin(const std::string& pluginFile,
                                 const std::string& optionsFile);
...
private:
    ...
    bool addObstacle(oppt::ObstacleSharedPtr& obstacle, bool executeCallbacks = true);

    bool removeObstacle(std::string obstacleName, bool executeCallbacks = true);

    bool changeObstaclePose(const std::string& name, const geometric::Pose& pose, bool executeCallbacks = true);
...
private:
    ...
    void onObstaclePoseChanged(const std::string& str, const geometric::Pose& pose);

    void onEnvironmentChanged(const EnvironmentChangeSharedPtr& environmentChange);

    std::function<void(const std::string&)> onAddObstacleFn_;

    std::function<void(const std::string&)> onRemoveObstacleFn_;

    std::function<void(const std::string&, const geometric::Pose&)> onObstaclePoseChangedFn_ = NULL;
