...
GeometrySharedPtr Mesh::copy() const
{
    std::string name = name_;
    Pose worldPose(worldPose_);    
    std::string meshFile = meshFile_;
    VectorFloat scale = scale_;
    GeometrySharedPtr copiedGeometry(new Mesh(name, meshFile, worldPose, scale));
    copiedGeometry->setColor(color_);
    return copiedGeometry;
}

std::shared_ptr<geometric::Geometry> Mesh::shallowCopy() const
{
    GeometrySharedPtr copiedGeometry(new Mesh());
    Mesh* mesh = static_cast<Mesh*>(copiedGeometry.get());
    std::string name = name_;
    Pose worldPose(worldPose_);    
    copiedGeometry->setName(name);
    copiedGeometry->setWorldPose(worldPose);    
    copiedGeometry->setColor(getColor());
    mesh->meshFile_ = meshFile_;
    mesh->scale_ = scale_;
    return copiedGeometry;
}
...
void Mesh::createCollisionGeometry()
{
    ...
    collisionGeometry_ = CollisionGeometrySharedPtr(new fcl::BVHModel<fcl::OBBRSS>());
