...
void SceneImpl::addObstacle(oppt::ObstacleSharedPtr& obstacle, bool executeCallback)
{
    ...
    if (obstacle->isEnabled()) {
        ...
        sceneCollisionManager_->registerObject(obstacle->getOpptCollisionObject()->getCollisionObject().get());
        sceneCollisionManager_->update();
        collisionObjectMap_[obstacle->getOpptCollisionObject().get()] = obstacle->getName();
    }
...
bool SceneImpl::removeObstacle(std::string obstacleName, bool executeCallback)
{
    ...
    for (size_t i = 0; i < obstacles_.size(); i++) {
        ...
        if (found) {
            ...
            if (managedIndex > -1) {
                sceneCollisionManager_->unregisterObject(obstacles_[i]->getOpptCollisionObject()->getCollisionObject().get());
                sceneCollisionManager_->update();
                obstaclesInManager_.erase(obstaclesInManager_.begin() + managedIndex);
            }

            if (collisionObjectMap_.find(obstacles_[i]->getOpptCollisionObject().get()) != collisionObjectMap_.end())
                collisionObjectMap_.erase(obstacles_[i]->getOpptCollisionObject().get());
...
bool SceneImpl::changeObstaclePose(const std::string& name, const geometric::Pose& pose, bool executeCallback)
{
    ...
    if (obstacle) {
        ...
        if (managedIndex > -1) {
            poseChanged = true;
            sceneCollisionManager_->unregisterObject(obstacle->getOpptCollisionObject()->getCollisionObject().get());           
            obstacle->setWorldPose(pose);            
            obstacle->updateCollisionObject();
            auto trans = obstacle->getOpptCollisionObject()->getCollisionObject()->getTranslation();
            sceneCollisionManager_->registerObject(obstacle->getOpptCollisionObject()->getCollisionObject().get());
            sceneCollisionManager_->update();
            collisionObjectMap_[obstacle->getOpptCollisionObject().get()] = obstacle->getName();
        } else {
...
void SceneImpl::makeContinuousCollisionReport(oppt::CollisionReportSharedPtr& collisionReport)
{
    ...
    for (size_t i = 0; i < obstacles_.size(); i++) {
        ...
        CollisionObjectSharedPtr obstacleCollisionObject = obstacles_[i]->getOpptCollisionObject()->getCollisionObject();
...
void SceneImpl::setChangeObstaclePoseCallback(std::function<void(const std::string&, const geometric::Pose&)> callbackFunction)
{
...
void SceneImpl::setRobotCollisionObjects(VectorCollisionObjects& robotCollisionObjects)
{
    ...
    for (auto & robotCollisionObject : robotCollisionObjects_) {
        ...
        collisionObjectMap_[robotCollisionObject.get()] = robotCollisionObject->getName();
    }
}
