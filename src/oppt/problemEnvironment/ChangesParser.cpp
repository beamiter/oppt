                                 ...
                                 std::unordered_map<unsigned int, VectorEnvironmentChanges>& changes)
{
    ...
    while (!infile.eof()) {
        ...
        if (output.find("add") != std::string::npos) {
            ...
            if (parseAddObstacle(infile, sdfString)) {		
                auto addObstacleChange = std::make_shared<ObstacleAddedChange>(sdfString, true);
                changes.at(t).push_back(addObstacleChange);
            }
        } else if (output.find("remove") != std::string::npos) {
            std::string obstacleName = "";
            if (parseRemoveObstacle(infile, obstacleName)) {
                auto removeObstacleChange = std::make_shared<ObstacleRemovedChange>(obstacleName, true);
                changes.at(t).push_back(removeObstacleChange);
            }
        } else if (output.find("changePose") != std::string::npos) {
            std::string obstacleName = "";
            VectorFloat poseVec;
            if (parseChangePose(infile, obstacleName, poseVec)) {
                geometric::Pose pose;
                ...
                auto changePoseChange = std::make_shared<ObstaclePoseChange>(obstacleName, pose, true);
...
bool ChangesParser::parseChangePose(std::ifstream& ifstream, std::string& obstacleName, VectorFloat& poseVec)
{
    std::string output;
    ifstream >> output;
    if (output.empty()) {
        WARNING("Empty obstacle name for change obstacle  pose change. Ignoring");
        return false;
    }
    ...
    for (size_t i = 0; i != 6; ++i) {
        ifstream >> output;
        if (output.empty()) {
            WARNING("Malformed pose for change obstacle pose change. Ignoring");
            return false;
        }
...
bool ChangesParser::parseRemoveObstacle(std::ifstream& ifstream, std::string& obstacleName)
{
    std::string output;
    ifstream >> output;
    if (output.empty()) {
        WARNING("Empty obstacle name for remove obstacle change. Ignoring");
        return false;
    }
...
bool ChangesParser::parseAddObstacle(std::ifstream& ifstream, std::string& sdfString)
{
    std::string output;
    ifstream >> output;
    if (output.empty()) {
