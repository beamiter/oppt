...
struct ProblemEnvironmentOptionsParser {
    ...
    ProblemEnvironmentOptionsPtr parseOptions(int argc, char const* argv[]) {
        ...
        if (!resources::FileExists(options_->robotPath)) {
            ERROR("Robot file '" + options_->robotPath + "' doesn't exist");
        }

        options_->robotPath = resources::FindFile(options_->robotPath);
...
public:
    ...
    int setup(int argc, char const* argv[]) {
        try {
            ...
            std::function<std::string(const std::string&)> cb = [](const std::string & file) {
                ...
                std::string fileName = elems[1] + ".sdf";
                if (!resources::FileExists(fileName))
                    ERROR("Robot model '" + fileName + "' doesn't exist");
                std::string path = resources::FindFile(fileName);
                VectorString elems2;
                split(path, fileName, elems2);
                return elems2[0];
            };
    ...
    void updateViewer(const RobotStateSharedPtr& currentState,
        ...
        VectorFloat particleColor = problemEnvironmentOptions_->particleColor;
        if (particleColor.size() != 4) {
            particleColor = VectorFloat({0.2, 0.6, 0.2, 0.1});
        }
    ...
    std::unique_ptr<oppt::RobotEnvironment> makeRobotEnvironment(OptionsType* options,
        ...
        if (prefix == "planning") {
            ...
            if (!(options->deactivateVisualization)) {
                robot_environment->getRobot()->initializeViewer_(options->robotPath,
