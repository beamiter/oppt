...
public:
    ...
    virtual HeuristicPluginSharedPtr clone(RobotEnvironment* const robotEnvironment) const override {
        std::shared_ptr<HeuristicPlugin> clonedPlugin(new RocksampleHeuristicPlugin());
...
private:
    void getRockLocations() {
        ...
        VectorObstaclePtr obstacles = scene->getObstacles();        
        unsigned int idx = 2;
        for (auto & obstacle : obstacles) {
            if (obstacle->isEnabled()) {
                auto pose = obstacle->getCollisionGeometry()->getWorldPose();
